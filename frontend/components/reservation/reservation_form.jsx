...
class ReservationForm extends React.Component {
  ...
  timePickerBuilder() {
    ...
    let timeArr = [];
    let openTime = this.props.houses[this.state.house_id ].open_time;
    openTime = (parseInt(openTime.split("T")[1].split(":")[0])) ;
    let closeTime = this.props.houses[this.state.house_id].close_time;
  ...
  reservationCheck(){
    let currentHouse = this.props.house
    let resArray = Object.values(this.props.reservations)

    for (let i = 0; i < resArray.length; i++){
      if (resArray[i].house.id === currentHouse.id){
        return true
      }
    }
    return false    
        ...
        <form>
          ...
          {this.props.currentUser ? (
            <p className="offer">
              ...
              {parseInt(this.state.house_id.slice(0, 1)) * 2 + 45} times
              today.
            </p>
          ) : (
            <p id="rese-log">Log in to reserve a visit!</p>
          )}
        </form>
      </div>
    );
